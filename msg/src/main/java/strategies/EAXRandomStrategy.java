...
public class EAXRandomStrategy {
	private Tour tourA;
	private Tour tourB;
	...
	public EAXRandomStrategy(Tour tourA, Tour tourB) {
		this.tourA = tourA;
		this.tourB = tourB;
	...
	public List<List<List<Integer>>> createABCycles() {
		...
		for (int j = 0; j < numOfChildren; ++j) {
			...
			tempTourB = mapTourB.entrySet().stream()
                    .collect(Collectors.toMap(Map.Entry::getKey, e -> new ArrayList<Integer>(e.getValue())));
			int numOfVertexes = tempTourA.size() * 2;
			List<List<Integer>> abcycles = new ArrayList<List<Integer>>(size);
			int[] randomNum = ThreadLocalRandom.current().ints(1, size).distinct().limit(size - 1).toArray();
			for (int i = 0; numOfVertexes > 0 && i <randomNum.length; ++i) {

				if (tempTourA.containsKey(randomNum[i])) {
					List<Integer> abcycleTemp = new ArrayList<Integer>(numOfVertexes);
						abcycleTemp = createABCycle(randomNum[i], new ArrayList<Integer>());
					if (!abcycleTemp.isEmpty() ) {
						numOfVertexes -= abcycleTemp.size() - 1;
						if(abcycleTemp.size()>5 && random.nextBoolean()) {
			...
			if(numOfVertexes>0) {
					List<Integer> keys = new ArrayList<Integer>(tempTourA.keySet());
					for (Integer i : keys) {
						if (tempTourA.containsKey(i)) {
							List<Integer> abcycleTemp = new ArrayList<Integer>(numOfVertexes);
								abcycleTemp = createABCycle(i, new ArrayList<Integer>());
							if (!abcycleTemp.isEmpty() ) {
								numOfVertexes -= abcycleTemp.size() - 1;
								if(abcycleTemp.size()>3 && random.nextBoolean()) {
									abcycles.add(abcycleTemp);
								}
							}
						}
